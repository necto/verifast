...
module Assertions(VerifyProgramArgs: VERIFY_PROGRAM_ARGS) = struct
  ...
  let rec assert_expr lemmas_in_scope (arg_pats,arg_names) retry_cont depth env e h env l msg url = 
    let t = eval None env e in
    if query_term t then
      ()
    else begin
      ignore (assert_expr_split lemmas_in_scope (arg_pats,arg_names) retry_cont depth e h env l msg url); ()
    end
  ...
  and assert_expr_split (lemmas_in_scope: (string * func_info) list) (arg_pats,arg_names) retry_cont depth e h env l msg url = 
    match e with
      IfExpr(l0, con, e1, e2) -> 
        branch
           (fun () -> assume (eval None env con) (fun () -> assert_expr_split lemmas_in_scope (arg_pats,arg_names) retry_cont depth e1 h env l msg url))
           (fun () -> assume (ctxt#mk_not (eval None env con)) (fun () -> assert_expr_split lemmas_in_scope (arg_pats,arg_names) retry_cont depth e2 h env l msg url))
    | WOperation(l0, And, [e1; e2], t) ->
      branch
        (fun () -> assert_expr_split lemmas_in_scope (arg_pats,arg_names) retry_cont depth e1 h env l msg url)
        (fun () -> assert_expr_split lemmas_in_scope (arg_pats,arg_names) retry_cont depth e2 h env l msg url)
    | _ -> with_context (Executing (h, env, expr_loc e, "Consuming expression")) (fun () -> assert_term lemmas_in_scope (arg_pats,arg_names) retry_cont depth e (eval None env e) h env l msg url; SymExecSuccess)
  ...
  *)
  ...
  let rec consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p checkDummyFracs coef cont_with_post =
    ...
    | ExprAsn (l, WOperation (lo, Eq, [WVar (lx, x, LocalVar); e], t)) ->
      begin match try_assoc x env with
        Some t -> assert_term lemmas_in_scope (arg_pats,arg_names) retry_cont depth p (ctxt#mk_eq t (ev e)) h env l "Cannot prove condition." None; cont [] h ghostenv env env' None
      | None -> let binding = (x, ev e) in cont [] h ghostenv (binding::env) (binding::env') None
      end
    | ExprAsn (l, e) ->
      assert_expr lemmas_in_scope (arg_pats,arg_names) retry_cont depth env e h env l "Cannot prove condition." None; cont [] h ghostenv env env' None
    | WMatchAsn (l, e, pat, tp) ->
      let v = ev e in
    ...
    | Sep (l, p1, p2) ->
      consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p1 checkDummyFracs coef (fun chunks h ghostenv env env' size post ->
        if post <> None then static_error l "Left-hand operand of separating conjunction cannot specify a postcondition." None;
        consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p2 checkDummyFracs coef (fun chunks' h ghostenv env env' _ post ->
    ...
    | IfAsn (l, e, p1, p2) ->
      ...
      branch
        (fun _ ->
           assume (ev e) (fun _ ->
             consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p1 checkDummyFracs coef cont_with_post))
        (fun _ ->
           assume (ctxt#mk_not (ev e)) (fun _ ->
             consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p2 checkDummyFracs coef cont_with_post))
    | WSwitchAsn (l, e, i, cs) ->
      let cont_with_post chunks h ghostenv1 env1 env'' _ post =
        let ghostenv, env =
          if post = None then ghostenv, env else ghostenv1, env1
      ...
      let rec iter cs =
        match cs with
          ...
          in
          branch
            (fun _ -> assume_eq t (mk_app ctorsym xs) (fun _ -> consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h (pats @ ghostenv) (xenv @ env) env' p checkDummyFracs coef cont_with_post))
            (fun _ -> iter cs)
        | [] -> success()
      in
      iter cs
    ...
    | ForallAsn (l, ManifestTypeExpr(_, tp), i, e) -> 
      ...
      assert_expr lemmas_in_scope (arg_pats,arg_names) retry_cont depth ((i, fresh_term) :: env) e h ((i, fresh_term) :: env) l "Cannot prove condition." None;
      cont [] h ghostenv env env' None
  ...
  let rec consume_asn_core lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p checkDummyFracs coef cont =
    consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env env' p checkDummyFracs coef $. fun chunks h ghostenv env env' size_first post ->
    cont chunks h ghostenv env env' size_first
  
  let consume_asn lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env p checkDummyFracs coef cont =
    consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env [] p checkDummyFracs coef $. fun chunks h ghostenv env env' size_first post ->
    cont chunks h ghostenv env size_first

  let rec consume_asn_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env p checkDummyFracs coef cont =
    consume_asn_core_with_post lemmas_in_scope (arg_pats,arg_names) retry_cont depth rules tpenv h ghostenv env [] p checkDummyFracs coef $. fun chunks h ghostenv env env' size_first post ->
