...
module VerifyProgram(VerifyProgramArgs: VERIFY_PROGRAM_ARGS) = struct
  ...
  module CheckFile(CheckFileArgs: CHECK_FILE_ARGS) = struct
    ...
    let verify_expr retry_cont readonly (pn,ilist) tparams pure leminfo funcmap sizemap tenv ghostenv h env xo e cont econt =
      verify_expr retry_cont (readonly, readonly) (pn,ilist) tparams pure leminfo funcmap sizemap tenv ghostenv h env xo e cont econt
    ...
    let rec verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env s tcont return_cont econt =
      ...
      let retry_vstmt_cont sizemap tenv ghostenv h env =
      ...
      in
      let retry_cont lemma_sttmt depth =
        verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env lemma_sttmt retry_vstmt_cont return_cont econt
      in
      ...
      let eval_h_core sideeffectfree pure h env e cont =
        ...
        verify_expr retry_cont sideeffectfree (pn,ilist) tparams pure leminfo funcmap sizemap tenv ghostenv h env None e cont econt
      in
      ...
      let verify_expr readonly h env xo e cont =
        if not pure then check_ghost ghostenv l e;
        verify_expr retry_cont readonly (pn,ilist) tparams pure leminfo funcmap sizemap tenv ghostenv h env xo e cont
      in
      ...
      | Assert (l, p) ->
        ...
        begin match wp with
             ...
             *                match reducible_exprs true (List.map fst params) [] postcond we with
             *                | None ->
             *                  printf "NO match\n";
             *                  None
             ...
             *         ) params
             *       in
             *       ExprStmt (CallExpr (le, name, [], [], (List.map (fun arg -> LitPat arg) args), Static))
             *   in
             *   verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env lemma_sttmt (fun sizemap tenv ghostenv h env->
             *       verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env s tcont return_cont econt)
             *     return_cont econt
             * end else begin *)
    ...
    and
      ...
      | s::ss ->
        with_context (Executing (h, env, stmt_loc s, "Executing statement")) (fun _ ->
            printf "verifying loc: %s\n" (string_of_loc (stmt_loc s));
            begin match s with
            | ExprStmt e ->
              printf "verifying E sttmt: %s\n" (string_of_expr e);
            | _ -> ();
            end;
      ...
      with_context (Executing (h, env, l, "Executing return statement")) $. fun () ->
        ...
        begin fun cont ->
          ...
          | Some e ->
            ...
            let retry_vrstmt_cont sizemap tenv ghostenv h env =
              verify_return_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env explicit l eo epilog return_cont econt
            in
            let retry_cont lemma_sttmt depth =
              verify_stmt (pn,ilist) blocks_done lblenv tparams boxes pure leminfo funcmap predinstmap sizemap tenv ghostenv h env lemma_sttmt retry_vrstmt_cont return_cont econt
            in
            verify_expr retry_cont false (pn,ilist) tparams pure leminfo funcmap sizemap tenv ghostenv h env None w (fun h env v ->
                cont h (Some v)) econt
        end $. fun h retval ->
          begin fun cont ->
                          ...
                          if cn = "java.lang.Object" then
                            ...
                            | Some (CtorInfo (lc0, xmap0, pre0, pre_tenv0, post0, epost0, terminates0, ss0, v0)) ->
                              with_context (Executing (h, env, lm, "Implicit superclass constructor call")) $. fun () ->
                                ...
                                let eval_h h env e cont = verify_expr no_retry_cont false (pn,ilist) [] false leminfo funcmap sizemap tenv ghostenv h env None e cont econt in
                                let pats = (List.map (fun e -> SrcPat (LitPat e)) args) in
                                verify_call no_retry_cont funcmap eval_h lm (pn, ilist) None None [] pats ([], None, xmap0, ["this", this], pre0, post0, Some(epost0), terminates0, v0) false is_upcall (Some supercn) leminfo sizemap h [] tenv ghostenv env (fun h env _ ->
                                    cont h) econt
                        end $. fun h ->
                              ...
                              if binding = Instance then begin
                                ...
                                | Some(e) -> 
                                  with_context (Executing (h, [], expr_loc e, "Executing field initializer")) $. fun () ->
                                    verify_expr no_retry_cont false (pn,ilist) [] false leminfo funcmap sizemap [(current_class, ClassOrInterfaceName cn); ("this", ObjType cn); (current_thread_name, current_thread_type)] ghostenv h [("this", this); (current_thread_name, List.assoc current_thread_name env)] None e (fun h _ initial_value ->
